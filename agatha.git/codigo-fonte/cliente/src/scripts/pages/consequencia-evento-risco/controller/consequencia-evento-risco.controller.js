...
(function(){
    'use strict';
    angular.module('gestaoRiscosApp').controller('ConsequenciaEventoRiscoController', ['$scope', '$mdToast', '$mdDialog', '$q', 'ConsequenciaEventoRiscoService', 'replace-string', controller]);
    ...
    function controller($scope, $mdToast, $mdDialog, $q, service, replaceString){
        ...
        scope.getDescricaoBySearch = function(){
            scope.replaceString();
            if(scope.registro.descricao && scope.registro.descricao.length >= 3){
                if(scope.registro.descricao.length >= 1000){
                    scope.registro.descricao = scope.registro.descricao.substring(0, 1000);
                    //noinspection JSUnresolvedFunction
                    $mdToast.show(
                        $mdToast.simple()
                ...
                service.getDescricaoBySearch(scope.registro.descricao).then(function(objectReturn){
