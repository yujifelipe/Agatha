...
(function () {
    'use strict';
    angular.module('gestaoRiscosApp').controller('MacroprocessoController', ['$scope', '$mdToast', '$mdDialog', '$q', 'MacroprocessoService', 'OrgaoService', 'replace-string', controller]);

    function controller($scope, $mdToast, $mdDialog, $q, service, OrgaoService, replaceString) {
        var scope = this;

        scope.init = function () {
            scope.getSecretarias(function () {
        ...
        scope.getSecretarias = function (callback) {
            OrgaoService.getSecretarias().then(function (response) {
                scope._orgaos = angular.copy(response.data);
                if (callback) {
                    callback();
                }
            });
