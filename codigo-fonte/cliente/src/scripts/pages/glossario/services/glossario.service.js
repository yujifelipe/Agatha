...
(function(){
    'use strict';
    angular.module('gestaoRiscosApp').factory('GlossarioService', ['$http', service]);

    function service($http){
        var baseUrl = '/gestaoriscos/api/glossarios';

        var findByFilter = function(filtros, query){
            var endPoint = baseUrl + "?page=" + filtros.page + "&size=" + filtros.size + "&termo=";
            if(query.termo != null){
                endPoint += query.termo;
            }
            endPoint += "&descricao=";
        ...
        var getTermoBySearch = function(termo){
            return $http.get(baseUrl + "/searchtermo?termo=" + termo);
        };
        ...
        return {
            ...
            getTermoBySearch: getTermoBySearch,
